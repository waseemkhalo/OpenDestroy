<script lang="ts">
 ...
 import {parseSavedLink,savedLinkFromDraft,savedLinkRejectionMessage,type SavedLinkDraft} from './lib/dictation/savedLink';
 ...
 let vars=$state('{}'),deleteConfirm=$state(false),connecting=$state(false);
 let savedLinks=$state<Link[]>([]),linkInput=$state(''),linkDraft=$state<SavedLinkDraft|null>(null),linkName=$state(''),linkKeywords=$state(''),linkNotice=$state('');
 ...
 async function boundary(){connectionRevision++;setAccount(null);connectedServer='';user=null;target=null;needsAccessibility=false;error='';vars='{}';savedLinks=[];discardLink();favorites=[];mediaStatus=null;token='';deleteConfirm=false;clearDictationMemory();resetDictationComposerCache();await cancel();}
 ...
 async function loadExtras(){
  ...
  vars=JSON.stringify(v,null,2);savedLinks=Array.isArray(l)?l as Link[]:[];favorites=f.favorites;
 ...
 async function saveExtras(){const owner=account();try{await api('PUT','/v1/variables',JSON.parse(vars),owner);if(owns(owner))message='Variables saved.';}catch(e){if(owns(owner))fail(e)}}
 function beginLink(){const parsed=parseSavedLink(linkInput);if(!parsed.ok){linkDraft=null;linkNotice=savedLinkRejectionMessage(parsed.reason);return;}linkDraft=parsed.draft;linkName=parsed.draft.name;linkKeywords=parsed.draft.keywords;linkNotice=parsed.draft.name?'':'Recognized '+parsed.draft.source+'. This link carries no title, so name it yourself.';}
 function discardLink(){linkDraft=null;linkInput='';linkName='';linkKeywords='';linkNotice='';}
 async function commitLink(){if(!linkDraft)return;const prepared=savedLinkFromDraft(linkDraft,linkName,linkKeywords);if(!prepared.ok){linkNotice=prepared.message;return;}
  // Re-pasting a link already saved renames it rather than duplicating a
  // URL the picker would then offer twice.
  const next=[...savedLinks.filter(l=>l.url!==prepared.link.url),prepared.link];if(next.length>100){linkNotice='Maximum 100 saved links.';return;}
  const owner=account();try{await api('PUT','/v1/links',next,owner);if(!owns(owner))return;savedLinks=next;discardLink();message='Saved link added.';}catch(e){if(owns(owner))fail(e)}}
 async function removeLink(url:string){const owner=account(),next=savedLinks.filter(l=>l.url!==url);try{await api('PUT','/v1/links',next,owner);if(owns(owner))savedLinks=next;}catch(e){if(owns(owner))fail(e)}}
...
<main class:compact={!expanded} style={`--camera-width:${camera.width}px;--camera-height:${camera.height}px`}>
 ...
 {#if user}<div class="settings-group"><h2>Snippet variables</h2><label>Variables (JSON)<textarea bind:value={vars} spellcheck="false" rows="4"></textarea></label><p class="hint">For example, {`{"my_company":"Your company"}`}. Only variables you define are used.</p><button onclick={()=>void saveExtras()}>Save variables</button></div><div class="settings-group"><h2>Saved links</h2><p class="hint">Paste a share link and give it a name you would say out loud. Speaking that name inserts the link. Destroy never uploads the file or changes who can open it.</p><label>Link<input bind:value={linkInput} type="text" spellcheck="false" autocomplete="off" placeholder="https://docs.google.com/document/d/…" onkeydown={e=>{if(e.key==='Enter'){e.preventDefault();beginLink();}}}/></label>{#if linkDraft}<label>Name<input bind:value={linkName} placeholder="What you would say out loud"/></label><label>Keywords<input bind:value={linkKeywords} spellcheck="false" placeholder="Other words you might say"/></label><p class="hint">Recognized as {linkDraft.source}.</p><button onclick={()=>void commitLink()}>Save link</button><button onclick={()=>discardLink()}>Cancel</button>{:else}<button onclick={()=>beginLink()}>Add link</button>{/if}{#if linkNotice}<p class="hint">{linkNotice}</p>{/if}{#each savedLinks as link (link.url)}<div class="row"><span>{link.name}</span><button onclick={()=>void removeLink(link.url)}>Remove</button></div>{/each}{#if !savedLinks.length}<p class="hint">No saved links yet.</p>{/if}</div><div class="settings-group"><h2>GIFs & stickers</h2><div class="row"><span>{mediaStatus?mediaStatus.state==='enabled'?'Available':mediaStatus.state==='unreachable'?'Backend unreachable':'Disabled on the backend':'Not checked yet'}</span><button onclick={()=>void checkMedia()}>Re-check</button></div>{#if mediaStatus&&mediaStatus.state!=='enabled'}<p class="hint">{dictationMediaUnavailableMessage(mediaStatus)}</p>{/if}<h2>Saved media favorites</h2>{#each favorites as favorite}<div class="row"><span>{favorite.title||'Saved media'}</span><button onclick={()=>void removeFavorite(favorite.id)}>Remove</button></div>{/each}<h2>Your data</h2><button onclick={()=>void exportData()}>Export personal data</button><button onclick={()=>deleteConfirm=true}>Delete personal data…</button>{#if deleteConfirm}<p>Deletes your saved preferences, snippets, favorites, links, vocabulary and usage from this backend and clears this app’s cache.</p><button class="danger" onclick={()=>void deleteData()}>Confirm deletion</button><button onclick={()=>deleteConfirm=false}>Keep data</button>{/if}</div>{/if}
