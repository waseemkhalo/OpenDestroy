...
export class Capture {
 ...
 private parts:Blob[]=[];private closeResume=()=>{};private cancelled=false;private stopRequested=false;private startPromise:Promise<void>|null=null;
 ...
 start(mic:string|null){this.startPromise=this.startInner(mic);return this.startPromise;}
 ...
 this.live=invokeAccount<{sessionId:string}>('destroy_dictation_stream_start',{sampleRateHz:ctx.sampleRate},this.owner).then(async r=>{if(this.cancelled){await invoke('destroy_dictation_stream_cancel',{sessionId:r.sessionId});return null;}return r.sessionId;}).catch(()=>null);
 let queued=0;
 ...
 if(queued>=64){this.liveError=true;return;}queued++;this.chain=this.chain.then(async()=>{const id=await this.live;if(id&&!this.cancelled&&!this.liveError)await invoke('destroy_dictation_stream_send_audio',{sessionId:id,audioBase64:bytesBase64(bytes)});}).catch(()=>{this.liveError=true;}).finally(()=>queued--);};
 ...
 if(id&&!this.cancelled){if(this.liveError){await invoke('destroy_dictation_stream_cancel',{sessionId:id}).catch(()=>{});}else{live=await invoke<{text:string;path:'cloud'}>('destroy_dictation_stream_finish',{sessionId:id}).catch(()=>null);}}
